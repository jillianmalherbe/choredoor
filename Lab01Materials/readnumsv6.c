//readnumsv6.c - Lab 01 - Jillian, Malherbe
...
// Notice we have started taking some coding shortcuts here
{
	...
	// Get here with pn pointing to a filled-in structure, or with an equal
...
void delete_data (int d)
{
	...
	struct node	*prev; //second pointer to keep track of previous node 
	ptemp = phead; // Setting temporary pointer equal to head node
	prev = NULL;
	// If list is empty
	if (pback == NULL)
		return;
	// If number is only one in list
	if (ptemp->pnextnode == pback && ptemp->datavalue == d) 
	{
		phead->pnextnode = pback; // Pointing head node to back node
		free(ptemp); // To free memory
		return;
	}
	// If number is at begining
	if (ptemp != NULL && ptemp->datavalue == d)
	{
		phead = ptemp->pnextnode; //changing head pointer
		free(ptemp); // To free memory
		return;
	}
	...
	while (ptemp != NULL)
	{
		//If number is last node in the list
		if (ptemp->datavalue == d && ptemp->pnextnode == NULL)
		{
			pback = prev;
			prev->pnextnode = NULL; //link previous node to back
			ptemp = NULL; 
			free(ptemp); // To free memory
			return;
		}
		//If number is in the middle
		if (ptemp->datavalue == d)
		{
			prev->pnextnode = ptemp->pnextnode; //link previous node to node following ptemp
			free(ptemp); // To free memory
			return;
		}
		prev = ptemp;
	...
	// If integer is not in linked list
	if (ptemp == NULL)
		return;
}
